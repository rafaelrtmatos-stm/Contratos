    ...
    <div className="space-y-1">
      ...
      <div className="relative">
        <input
          ...
          maxLength={9}
          disabled={loading}
          className="w-full px-3 py-2 pr-9 text-xs border border-slate-300 rounded-lg bg-white text-slate-900 placeholder-slate-400 font-bold
            focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500
            disabled:bg-slate-100 disabled:cursor-not-allowed transition-all"
        />
        {loading && (
          <Loader className="w-4 h-4 animate-spin text-slate-400 absolute right-2.5 top-1/2 -translate-y-1/2" />
        )}
      </div>

      {/* Botão para buscar CEP por nome da rua/cidade */}
      <button
        type="button"
      ...
      >
        ...
        Não sei o CEP, buscar pelo nome da rua
      </button>
