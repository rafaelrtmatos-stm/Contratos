 ...
 */
export type PapelContratual = 'contratado' | 'contratante' | 'vendedor' | 'comprador' | 'locador' | 'locatario';
...
const ROTULOS: Record<PapelContratual, { M: string; F: string; neutro: string }> = {
  ...
  locador: { M: 'LOCADOR', F: 'LOCADORA', neutro: 'LOCADOR(A)' },
  locatario: { M: 'LOCATÁRIO', F: 'LOCATÁRIA', neutro: 'LOCATÁRIO(A)' },
};
