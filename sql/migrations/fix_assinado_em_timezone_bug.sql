-- ============================================================
-- Corrige "hora adiantada" no manifesto/tela após assinatura via
-- link.
--
-- Bug: v_assinado_em estava declarada como TIMESTAMP (sem fuso) em
-- vez de TIMESTAMPTZ. O Postgres devolvia o valor de assinado_em
-- (que é timestamptz no banco) já sem informação de fuso; o front
-- então interpretava essa string como horário local e aplicava o
-- ajuste de fuso de novo -> soma dupla, hora aparece adiantada.
--
-- Fix: v_assinado_em agora é TIMESTAMPTZ, preservando o fuso vindo
-- do banco (coluna assinado_em é timestamptz). Todo o resto da
-- função é idêntico a prevent_duplicate_role_signature.sql.
--
...
BEGIN
  ...
  -- TRAVA: essa parte (o cliente que assina por link) já tem
